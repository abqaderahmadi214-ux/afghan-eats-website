...
window.submitRestaurantClaim = async function submitRestaurantClaim(event) {
  ...
  try {
    ...
    const emailNote=result.emailVerified?claimText('Your email is verified.','ایمیل شما تأیید شده است.'):(result.verificationEmailSent?claimText('A verification link was sent to your email. Email verification is required before ownership approval.','لینک تأیید به ایمیل شما ارسال شد. تأیید ایمیل پیش از تأیید مالکیت لازم است.'):claimText('The claim was saved, but the verification email could not be sent yet.','درخواست ذخیره شد، اما ایمیل تأیید فعلاً ارسال نشد.'));
    const phoneNote=claimText('Afghan Eats will also verify the claim through a published restaurant number before granting owner access.','افغان ایتس پیش از دادن دسترسی مالک، درخواست را از طریق یکی از شماره‌های عمومی رستورانت نیز بررسی می‌کند.');
    output.innerHTML = `<b>${claimEsc(result.duplicate ? claimText('Existing claim found', 'درخواست قبلی پیدا شد') : claimText('Claim received', 'درخواست دریافت شد'))}</b><p>${claimEsc(result.restaurantName || AECLAIM.restaurant?.name || '')}</p><p>${claimEsc(claimText('Reference:', 'شماره پیگیری:'))} <strong>${claimEsc(result.reference)}</strong></p><small>${claimEsc(emailNote)}<br>${claimEsc(phoneNote)}</small>`;
