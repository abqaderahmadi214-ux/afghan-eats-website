...
async function trustLoadReviews(){
  ...
  if(!reviews.length){const emptyScore=box.querySelector('.review-score span');if(emptyScore)emptyScore.textContent=trustText('No reviews yet','هنوز نظری ثبت نشده')}
  const ratingSelect=box.querySelector('select[name="rating"]');if(ratingSelect)ratingSelect.innerHTML=`<option value="5">5 — ${trustText('Excellent','عالی')}</option><option value="4">4 — ${trustText('Very good','بسیار خوب')}</option><option value="3">3 — ${trustText('Good','خوب')}</option><option value="2">2 — ${trustText('Fair','متوسط')}</option><option value="1">1 — ${trustText('Poor','ضعیف')}</option>`;
